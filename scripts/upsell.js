...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/6a0b6cec7f4c8c35affcad7c-preview") ? value : (value.startsWith("/") ? "/6a0b6cec7f4c8c35affcad7c-preview" + value : "/6a0b6cec7f4c8c35affcad7c-preview/" + value);
  };
...
const processKlarnaUpsell = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `https://app-cms-api-proxy-dev-001.azurewebsites.net/vrio/order_offers`,
      {
        method: "POST",
        headers: {
          authorization: `appkey ${INTEGRATION_ID}`,
        ...
        body: JSON.stringify({
          ...
          order_id: lastOrderId,
          pageId: "MkdTJP1LailNM2YEHY421CCfoqAfUkKBBtEPyQjUU9WGRRbvenzdLMFyi32tCy1h"
        })
      }
    );
...
const processUpsell = async () => {
  ...
  try {
    const orderData = JSON.parse(sessionStorage.getItem("orderData"));
    orderData.pageId = "MkdTJP1LailNM2YEHY421CCfoqAfUkKBBtEPyQjUU9WGRRbvenzdLMFyi32tCy1h";
    const lastOrderId = sessionStorage.getItem("cms_oid");
    ...
    uniqueSelectedProductIds.forEach( (selectedProductId) => {
      ...
      if (product) {
        const shippableEl = document.querySelector(`[data-shippable-product-id="${product.id}"]`);
        shippingProfileId = resolveProfileIdFromEl(shippableEl, orderData?.ship_country);
        const bindedProductOfferData = getVrioOfferIdByProductId(product.id);
        const bindedProductOfferId =
          bindedProductOfferData.offerId ?? selectedProductOfferId;
        orderData.offers.push({
